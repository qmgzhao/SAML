...
from .model import Whisper, ModelDimensions, MoE
# from .model import Whisper, ModelDimensions
from .transcribe import transcribe
from .version import __version__
import loralib as lora
from .moe_layers_init import moe_layers_list
...
def load_moe_model(loadfrom, is_pretrain, lora_r, lora_alpha, lora_dropout, temperature, device):
    spk_list = ["19", "26", "39", "78", "83", "87", "89", "118", "163", "196"]  # 10 experts
    n_expert = len(spk_list)
    # 1. model
    ...
    model = Whisper(dims, moe).to(device)

    # 2. state_dict
    # 2.1 load whisper state_dict
    state_dict = torch.load(loadfrom).to(device).state_dict()
